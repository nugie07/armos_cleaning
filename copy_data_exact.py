#!/usr/bin/env python3
"""
Copy data from Database A to Database B with exact same table structure
"""
...
def copy_orders_exact(logger, start_date, end_date, warehouse_id):
    """Copy orders with exact same structure - no data type conversion needed"""
    logger.info("=== COPYING ORDERS (EXACT STRUCTURE) ===")
    ...
    try:
        ...
        while offset < total_orders:
            ...
            # Insert batch into target - exact same structure
            insert_query = """
...
def copy_order_details_exact(logger, start_date, end_date, warehouse_id):
    """Copy order details with exact same structure"""
    logger.info("=== COPYING ORDER DETAILS (EXACT STRUCTURE) ===")
...
def main():
    ...
    if len(sys.argv) != 4:
        logger.error("Usage: python3 copy_data_exact.py <start_date> <end_date> <warehouse_id>")
        logger.error("Example: python3 copy_data_exact.py 2025-04-01 2025-04-30 4512")
        sys.exit(1)
    ...
    try:
        logger.info("=== COPYING DATA WITH EXACT STRUCTURE ===")
        logger.info(f"Date Range: {start_date} to {end_date}")
        logger.info(f"Warehouse ID: {warehouse_id}")
        
        # Step 1: Copy orders
        copied_orders = copy_orders_exact(logger, start_date, end_date, warehouse_id)
        
        # Step 2: Copy order details
        copied_details = copy_order_details_exact(logger, start_date, end_date, warehouse_id)
